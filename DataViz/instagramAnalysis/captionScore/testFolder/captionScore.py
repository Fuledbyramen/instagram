...
def hashtagAmountScoreAndIncrease(captions):
	...
	return [score, likesIncrease, commentsIncrease]
...
def calculateIncreaseExpected(captions):
	...
	for caption in captions:
		...
		sumOfScores = [x + y for x, y in zip(sumOfScores, results)]
	try:
		sumOfScores = [x / len(captions) for x in sumOfScores]
		return sumOfScores
...
def calculateOverallCaptionScore(captions):
	...
	try:
		return sumOfScores/len(captions)
	except ZeroDevisionError:
		return 0
...
def run(username):
	...
	return [overallScore, overallLikeIncrease, overallCommentIncrease]
