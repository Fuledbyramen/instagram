...
cursor.execute('SELECT DISTINCT ON (imageid) date, likes FROM insta_posts LIMIT %s;', (limit,))
...
for day, hour in week.items():
    ...
    plt.plot(x_smooth, y_smooth, color="black", lw=3.5)
...
plt.axis('off')
plt.savefig('webGraph1.png', transparent=True)
plt.show()
